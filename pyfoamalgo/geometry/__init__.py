...
from ..lib.geometry import EPix100Geometry, JungFrauGeometry
from ..lib.geometry_1m import AGIPD_1MGeometry as _AGIPD_1MGeometry
from ..lib.geometry_1m import LPD_1MGeometry as _LPD_1MGeometry
from ..lib.geometry_1m import DSSC_1MGeometry as _DSSC_1MGeometry
...
class DSSC_1MGeometry(_DSSC_1MGeometry, _1MGeometryPyMixin):
    """DSSC_1MGeometry.
...
class LPD_1MGeometry(_LPD_1MGeometry, _1MGeometryPyMixin):
    """LPD_1MGeometry.
...
class AGIPD_1MGeometry(_AGIPD_1MGeometry, _1MGeometryPyMixin):
    """AGIPD_1MGeometry.
...
class JungFrauGeometry(JungFrauGeometry, _GeometryPyMixin):
    """JungFrauGeometry.
...
class EPix100Geometry(EPix100Geometry, _GeometryPyMixin):
    """EPix100Geometry.
...
def load_geometry(detector, *,
    ...
    if detector == "JungFrau":
        ...
        if stack_only:
            return JungFrauGeometry(*shape)

        return JungFrauGeometry.from_crystfel_geom(
            *shape, filepath)

    if detector == "ePix100":
        shape = module_grid_shape(n_modules, detector=detector)
        if stack_only:
            return EPix100Geometry(*shape)
...
def maybe_mask_asic_edges(image, detector):
    ...
    if detector == "JungFrau":
        JungFrauGeometry.mask_module_py(image)
        return

    if detector == "ePix100":
        EPix100Geometry.mask_module_py(image)
        return
